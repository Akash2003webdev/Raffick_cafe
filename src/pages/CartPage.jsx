...
export default function CartPage({ onToast, onOrderSent }) {
  useSEO({
    title: "Your Cart | Raffick Cafe",
    description: "Review your order from Raffick Cafe, Sattur before checkout.",
    path: "/cart",
    noindex: true,
  });
    ...
    return (
      <div className="max-w-md mx-auto px-6 pt-24 pb-32 text-center flex flex-col items-center justify-center min-h-[60vh]">
        <div className="w-20 h-20 rounded-[2rem] bg-gray-50 flex items-center justify-center border border-gray-100/80 mb-6 shadow-inner relative">
          ...
          <span className="absolute -top-1 -right-1 flex h-3 w-3 rounded-full bg-gold-400" />
        </div>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 md:pt-14 pb-40 md:pb-20 min-h-screen bg-gray-50/40">
      ...
      {blockedItems.length > 0 && (
        <div className="bg-gold-500/10 border border-gold-500/20 text-gold-900 text-xs md:text-sm rounded-2xl p-4 mb-6 flex items-start gap-3 backdrop-blur-sm animate-fadeUp">
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl border p-4 flex items-center gap-4 transition-all duration-300 ${
                  isBlocked
                    ? "border-gold-200/80 bg-gold-50/10 shadow-none"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-base font-extrabold text-gold-600 mt-1">
                    ₹{item.price}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 bg-gray-50/80 border border-gray-100 rounded-full p-1.5">
                  <button
                    ...
                    className="w-7 h-7 rounded-full bg-white shadow-md border border-gray-100 flex items-center justify-center transition-transform active:scale-90 hover:text-gold-600"
                  >
                    <Minus size={12} />
                  </button>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.03)] p-5 md:p-6 space-y-6 lg:sticky lg:top-28 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
                ...
                return (
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ${
                      active
                        ? "bg-gradient-to-r from-primary-600 to-primary-700 text-white shadow-md scale-[1.02]"
          ...
          <div className="space-y-4">
            {orderType === "Delivery" && (
              <div className="relative group">
                <MapPin size={16} className="absolute left-4 top-4 text-gray-400 group-focus-within:text-gold-500 transition-colors" />
                <textarea
                  placeholder="Full Delivery Address *"
                  value={address}
                  onChange={(e) => setAddress(e.target.value)}
                  rows={2}
                  className="w-full bg-gray-50/50 border border-gray-200/80 rounded-xl pl-11 pr-4 py-3 text-sm font-medium outline-none transition-all duration-300 focus:bg-white focus:border-gold-500 focus:ring-4 focus:ring-gold-500/10 resize-none"
                />
              </div>
            )}
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-gray-50/50 border border-gray-200/80 rounded-xl pl-11 pr-4 py-3 text-sm font-medium outline-none transition-all duration-300 focus:bg-white focus:border-gold-500 focus:ring-4 focus:ring-gold-500/10"
              />
            </div>

            <div className="relative group">
              <Phone size={16} className="absolute left-4 top-3.5 text-gray-400 group-focus-within:text-gold-500 transition-colors" />
              <input
                type="tel"
          ...
          <div className="hidden md:block border-t border-gray-100 pt-4">
            ...
            <button
              ...
              disabled={!canOrder}
              className="group w-full py-4 rounded-2xl bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-bold text-sm tracking-wide shadow-lg shadow-primary-500/10 disabled:opacity-40 disabled:pointer-events-none transition-all duration-300 flex items-center justify-center gap-2"
            >
              Order via WhatsApp
      ...
      <div className="fixed bottom-16 md:hidden left-0 right-0 bg-white/90 backdrop-blur-md border-t border-gray-100/80 p-4 z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.04)]">
        <div className="max-w-xl mx-auto flex items-center justify-between gap-6">
          ...
          <button
            onClick={() => setShowConfirm(true)}
            disabled={!canOrder}
            className="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-primary-600 to-primary-700 text-white font-bold text-sm tracking-wide shadow-md shadow-primary-500/10 disabled:opacity-40 transition-transform active:scale-95"
          >
            Order via WhatsApp
          </button>
        </div>
        ...
        <div className="space-y-4 text-sm">
          <div className="flex items-center justify-between bg-gray-50 p-3 rounded-xl border border-gray-100">
            <div>
              ...
              <span className="font-bold text-gold-600 text-sm">{orderType}</span>
            </div>
            ...
            {items.map((item) => (
              ...
              >
                <span className="font-medium max-w-[70%]">
                  {item.name} {item.variantName ? `(${item.variantName})` : ""} <span className="text-gold-600 font-bold">x{item.quantity}</span>
                </span>
          ...
          <div className="border-t border-gray-200/60 pt-3 flex justify-between items-center font-black text-gray-900 text-base">
            ...
            <span className="text-xl text-gold-600">₹{total}</span>
          </div>

          <div className="flex items-center gap-2 bg-gold-50/50 text-gold-800 p-3 rounded-xl border border-gold-100/60 text-xs font-medium">
