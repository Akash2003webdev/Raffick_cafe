...
export default function EnquiryPage({ onToast }) {
  useSEO({
    title: "Enquiry | Raffick Cafe - Bulk Orders & Catering",
    description:
      "Get in touch with Raffick Cafe, Sattur for bulk orders, catering, or general enquiries. Call 09003788247 or send us a message.",
    path: "/enquiry",
  });
  ...
  const canSubmit =
    form.name.trim() && form.phone.trim().length >= 10 && form.message.trim();
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 md:pt-14 pb-24 md:pb-16 min-h-screen bg-gray-50/50">
      ...
      <div className="relative rounded-[2rem] overflow-hidden h-44 md:h-60 bg-gradient-to-br from-primary-700 via-primary-900 to-gray-950 flex items-center justify-center mb-8 md:mb-12 shadow-xl border border-white/10">
        ...
        <div className="text-center px-4 z-10">
          ...
          <p className="text-xs md:text-base text-gray-300 mt-2 max-w-md mx-auto font-medium">
            Bulk orders, catering, or feedback — we'll quickly connect with you
            on WhatsApp
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative group">
          <MessageSquare
            size={18}
            className="absolute left-4 top-4 text-gray-400 group-focus-within:text-primary-500 transition-colors"
          />
          <textarea
      ...
      >
        <div className="space-y-3 text-sm bg-gray-50 p-4 rounded-2xl border border-gray-100">
          <div className="flex items-center justify-between pb-2 border-b border-gray-200/60">
            ...
            <span className="font-bold text-primary-600 bg-primary-50 px-2.5 py-0.5 rounded-lg text-xs">
              {form.enquiryType}
            </span>
          </div>
